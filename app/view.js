...
export function createView(s){
  ...
  const {libraryView,entrySize,previewLayout,rollerLayout}=s.preferences;
...
function previews() {
  const m = current();
  if(!m)return "";
  return `<div class="preview-heading"><div class="eyebrow">One definition / three states</div><div class="row wrap">${layoutToggle('preview',previewLayout)}</div></div><div class="state-grid layout-${previewLayout}">${states.map(s => stateCard(m,s)).join('')}</div><p class="state-footnote">Only applicable effects appear. Design notes stay off the cards.</p>`;
}
function toolbar() {
...
function workspace() {
  ...
  return `<div class="workspace-a"><section class="library-section"><div class="section-heading row spread wrap"><span class="eyebrow muted">Mutation library</span>${libraryControls()}</div><div class="selection-tools"><span data-selection-count>${s.bulk.size} selected</span><button class="small quiet" data-action="select-visible">Select visible</button><button class="small quiet" data-action="clear-selection">Clear</button><button class="small" data-action="export-selected" ${s.bulk.size?'':'disabled'}>Export selected</button></div><div class="library-pane" data-library="adaptive">${library('adaptive')}</div></section><section class="panel adjustable-editor"><div class="panel-head">${editorHeading()}</div><div class="panel-body" id="definition-fields">${editorForm()}</div><div class="preview-region" data-previews>${previews()}</div></section></div>`;
}
