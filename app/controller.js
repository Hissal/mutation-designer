...
function definitionReview(m){
  return `<h3>${esc(title(m))}</h3><p class="muted">${esc(m.workflow)} · ${esc(m.kind)} · ${esc(m.tags.join(', ')||'No tags')}</p>${['body','positiveTitle','positive','negativeTitle','negative','notes'].map(key=>`<dl><dt>${{body:'Bodily change',positiveTitle:'Positive title',positive:'Positive effect',negativeTitle:'Negative title',negative:'Negative effect',notes:'Design notes'}[key]}</dt><dd>${esc(m[key]||'—')}</dd></dl>`).join('')}<p class="muted">Visual: ${esc(m.visual)} · Icon: ${esc(m.kind)}</p>${m.image?`<img class="conflict-image" src="${esc(m.image)}" alt="Attached reference">`:'<p class="muted">No uploaded image</p>'}`;
}
...
dialog.addEventListener('change',event=>{
  if(event.target.id==='export-format'){const markdown=event.target.value==='markdown';document.querySelector('#notes-choice').hidden=!markdown;document.querySelector('#export-note').textContent=markdown?'Markdown contains readable definitions and state rules, without images.':'JSON always includes complete definitions and embedded reference images.';}
