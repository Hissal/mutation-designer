...
test('Markdown omits notes when requested and never embeds image payloads',()=>{const m=sample(),text=markdownExport([m],{includeNotes:false});assert.ok(!text.includes(m.notes));assert.ok(!text.includes(image));assert.ok(text.includes('Positive and negative'));assert.ok(markdownExport([m]).includes(m.notes));});
